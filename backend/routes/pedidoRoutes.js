...
const validacionPedido = [
  ...
  body('items.*.producto').notEmpty().withMessage('El ID del producto es requerido'),
  body('items.*.cantidad').isInt({ min: 1 }).withMessage('La cantidad debe ser mayor a 0'),
  body('direccionEnvio.direccion').notEmpty().withMessage('La dirección es requerida'),
  body('metodoPago').isIn(['efectivo', 'tarjeta', 'transferencia', 'pse']).withMessage('Método de pago inválido'),
  validarResultados
];
...
router.get('/estadisticas/general', proteger, autorizar('admin'), obtenerEstadisticas);
